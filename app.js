...
const SETTLEMENT_BUILD_REQUEST_KEY='roadprints-settlement-build-requests-v1';
const requestedSettlementBuilds=new Set(),settlementBuildRequestAttempts=new Set();
try{for(const code of JSON.parse(localStorage.getItem(SETTLEMENT_BUILD_REQUEST_KEY)||'[]'))requestedSettlementBuilds.add(code)}catch(_){}
function saveSettlementBuildRequests(){try{localStorage.setItem(SETTLEMENT_BUILD_REQUEST_KEY,JSON.stringify([...requestedSettlementBuilds]))}catch(_){}}
...
async function requestSettlementBuild(town){try{const settlements=await loadSettlementInventoryIndex(),entry=settlements.get(normaliseSettlementName(town));if(!entry||entry.status!=='pending'||requestedSettlementBuilds.has(entry.code)||settlementBuildRequestAttempts.has(entry.code))return;settlementBuildRequestAttempts.add(entry.code);const response=await fetch(API_BASE_URL+'/settlement-build-requests',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({settlements:[{code:entry.code,name:entry.name}]})});if(!response.ok)throw Error('Settlement queue unavailable');requestedSettlementBuilds.add(entry.code);saveSettlementBuildRequests()}catch(error){console.info('Shared settlement build request deferred:',error)}}
