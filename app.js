...
function renderSettlementCheck(){const list=document.getElementById('roadDiscoveryList'),old=document.getElementById('settlementMatchingCheck');if(!list)return;old?.remove();const roads=[...roadDiscoveryLedger.values()].filter(road=>road.category==='Local roads');if(!roads.length)return;const panel=document.createElement('details'),summary=document.createElement('summary'),rows=document.createElement('ul');panel.id='settlementMatchingCheck';panel.className='road-discovery-group settlement-matching-check';summary.textContent='Settlement matching check (test) · '+roads.length;panel.append(summary,rows);for(const road of roads.sort((a,b)=>a.label.localeCompare(b.label))){queueSettlementCheck(road);const result=settlementCheckResults.get(road.id),row=document.createElement('li'),label=document.createElement('strong'),state=document.createElement('span');label.textContent=road.label;const legacy=roadDiscoveryLocalityByBoundary(road);state.textContent=result?.error?String(result.error):!result?'Checking…':result.names.length?'ONS: '+result.names.join(', '):'ONS: no settlement';row.title='Legacy locality: '+legacy;row.append(label,state);rows.append(row)}list.before(panel)}
