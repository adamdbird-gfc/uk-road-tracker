...
function refreshBoundarySettlementMetrics(){const towns=new Map();for(const road of roadDiscoveryLedger.values())if(road.category==='Local roads'){const result=settlementCheckResults.get(road.id);if(!result||result.error)continue;for(const town of result.names)(towns.get(town)||towns.set(town,[]).get(town)).push(road)}for(const [town,roads] of towns)setTown(town,roads)}
