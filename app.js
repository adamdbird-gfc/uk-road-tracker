...
function setTown(town,roads){
 ...
 document.querySelectorAll('.road-discovery-town summary[data-town]').forEach(x=>{if(x.dataset.town!==town)return;const n=document.createElement('span'),m=document.createElement('button');n.className='road-discovery-town-name';n.textContent=town;m.className='road-discovery-town-metric';m.type='button';if(Number.isFinite(Number(inv?.count))){m.textContent=roads.length+' / '+inv.count+' · '+Math.round(roads.length/inv.count*100)+'%';m.disabled=true}else{m.textContent=state==='loading'?'Loading…':state==='failed'?'Failed · Retry':'Calculate';m.onclick=event=>{event.preventDefault();event.stopPropagation();void calculateTown(town,roads)}}x.replaceChildren(n,m)})}
async function calculateTown(town,roads){const key=townKey(town,roads[0].point);if(localTownInventoryStates.get(key)==='loading')return;localTownInventoryStates.set(key,'loading');setTown(town,roads);const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),70000);try{const response=await fetch(API_BASE_URL+'/local-road-inventory?lat='+encodeURIComponent(roads[0].point.lat)+'&lng='+encodeURIComponent(roads[0].point.lng),{signal:controller.signal});const inventory=await response.json().catch(()=>null);if(!response.ok||!Number.isFinite(Number(inventory?.count)))throw Error('Local road inventory unavailable');localTownInventories.set(key,inventory);saveLocalTownInventories();localTownInventoryStates.delete(key)}catch(error){console.warn('Town road-count lookup failed:',error);localTownInventoryStates.set(key,'failed')}finally{clearTimeout(timeout)}setTown(town,roads)}
