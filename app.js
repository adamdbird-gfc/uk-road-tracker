...
let settlementInventoryIndexPromise;
...
async function loadSettlementInventoryIndex(){if(!settlementInventoryIndexPromise)settlementInventoryIndexPromise=fetch(SETTLEMENT_INVENTORY_INDEX_URL,{cache:'force-cache'}).then(async response=>{if(!response.ok)throw Error('Settlement cache unavailable');const data=await response.json();return new Map((data.settlements||[]).flatMap(entry=>[entry.name,...(entry.aliases||[])].map(alias=>[normaliseSettlementName(alias),entry])))});return settlementInventoryIndexPromise}
async function fetchTownInventory(town){const settlements=await loadSettlementInventoryIndex(),entry=settlements.get(normaliseSettlementName(town));if(!entry)return null;const response=await fetch('settlement-inventories-v1/'+encodeURIComponent(entry.code)+'.json',{cache:'force-cache'});const inventory=await response.json().catch(()=>null);if(!response.ok||!Number.isFinite(Number(inventory?.count)))throw Error('Settlement inventory unavailable');return inventory}
