...
function runSettlementChecks(){while(settlementCheckWorkers<1&&settlementCheckQueue.length){const road=settlementCheckQueue.shift();settlementCheckWorkers++;(async()=>{try{const names=new Set();for(const evidence of road.evidence||[]){const response=await fetch(API_BASE_URL+'/settlements-for-geometry',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({geometry:evidence.geometry})});if(!response.ok)throw Error('Boundary check unavailable');const data=await response.json();for(const settlement of data.settlements||[])if(settlement.name)names.add(settlement.name);await new Promise(resolve=>setTimeout(resolve,250))}settlementCheckResults.set(road.id,{names:[...names]});saveSettlementCheckResults()}catch(_){settlementCheckResults.set(road.id,{error:true})}finally{settlementCheckPending.delete(road.id);settlementCheckWorkers--;renderRoadDiscovery();runSettlementChecks()}})()}}
