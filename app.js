...
function setTown(town,roads){
 ...
 document.querySelectorAll('.road-discovery-town summary[data-town]').forEach(x=>{if(x.dataset.town!==town)return;const details=x.parentElement,n=document.createElement('span'),m=document.createElement('button'),oldAction=details.querySelector('.town-count-map-action');n.className='road-discovery-town-name';n.textContent=town;m.className='road-discovery-town-metric';m.type='button';oldAction?.remove();if(Number.isFinite(Number(inv?.count))){m.textContent=roads.length+' / '+inv.count+' · '+Math.round(roads.length/inv.count*100)+'%';m.disabled=true;const action=document.createElement('button');action.className='town-count-map-action';action.type='button';action.textContent='Show count area on map';action.onclick=event=>{event.preventDefault();event.stopPropagation();showTownCountArea(town,roads[0].point,inv)};details.insertBefore(action,details.querySelector('ul'))}else{m.textContent=state==='loading'?'Loading…':state==='failed'?'Failed · Retry':'Calculate';m.onclick=event=>{event.preventDefault();event.stopPropagation();void calculateTown(town,roads)}}x.replaceChildren(n,m)})}
