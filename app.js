...
let townCountAreaLayer=null;
const LOCAL_TOWN_DEFAULT_RADIUS_KM=3.5;
const LOCAL_TOWN_FALLBACK_RADIUS_KM=2;
...
function setTown(town,roads){
 ...
 document.querySelectorAll('.road-discovery-town summary[data-town]').forEach(x=>{if(x.dataset.town!==town)return;const n=document.createElement('span'),m=document.createElement('button');n.className='road-discovery-town-name';n.textContent=town;m.className='road-discovery-town-metric';m.type='button';if(Number.isFinite(Number(inv?.count))){m.textContent=roads.length+' / '+inv.count+' · '+Math.round(roads.length/inv.count*100)+'%';m.disabled=true;const mapButton=document.createElement('button');mapButton.className='town-count-map-quick';mapButton.type='button';mapButton.textContent='Map';mapButton.onclick=event=>{event.preventDefault();event.stopPropagation();showTownCountArea(town,roads[0].point,inv)};x.replaceChildren(n,m,mapButton)}else{m.textContent=state==='loading'?'Loading…':state==='failed'?'Failed · Retry':'Calculate';m.onclick=event=>{event.preventDefault();event.stopPropagation();void calculateTown(town,roads)};x.replaceChildren(n,m)}})}
async function fetchTownInventory(point,radiusKm,signal){const url=API_BASE_URL+'/local-road-inventory?lat='+encodeURIComponent(point.lat)+'&lng='+encodeURIComponent(point.lng)+'&radius_km='+encodeURIComponent(radiusKm);const response=await fetch(url,{signal});const inventory=await response.json().catch(()=>null);if(!response.ok||!Number.isFinite(Number(inventory?.count)))throw Error('Local road inventory unavailable');return {...inventory,lookup:{lat:point.lat,lng:point.lng,radiusKm}}}
async function calculateTown(town,roads){const key=townKey(town,roads[0].point);if(localTownInventoryStates.get(key)==='loading')return;localTownInventoryStates.set(key,'loading');setTown(town,roads);const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),110000);try{let inventory,lastError;for(const radiusKm of [LOCAL_TOWN_DEFAULT_RADIUS_KM,2,1.5]){try{inventory=await fetchTownInventory(roads[0].point,radiusKm,controller.signal);break}catch(error){lastError=error}}if(!inventory)throw lastError||Error('Local road inventory unavailable');localTownInventories.set(key,inventory);saveLocalTownInventories();localTownInventoryStates.delete(key)}catch(error){console.warn('Town road-count lookup failed:',error);localTownInventoryStates.set(key,'failed')}finally{clearTimeout(timeout)}setTown(town,roads)}
function clearTownCountArea(){townCountAreaLayer?.clearLayers();document.getElementById('townCountMapReturn')?.remove();if(map)showDefaultUnitedKingdomView()}
function showTownCountArea(town,point,inventory){const context=inventory?.lookup||{lat:point.lat,lng:point.lng,radiusKm:LOCAL_TOWN_DEFAULT_RADIUS_KM},radiusKm=Number(context.radiusKm)||LOCAL_TOWN_DEFAULT_RADIUS_KM;mapRenderingRequested=true;activateRoadprintsScreen('map');const draw=()=>{initMap();if(!map||!window.L)return;townCountAreaLayer?.clearLayers();if(!townCountAreaLayer)townCountAreaLayer=L.layerGroup().addTo(map);const circle=L.circle([context.lat,context.lng],{radius:radiusKm*1000,color:'#f7c450',weight:2,opacity:.95,fillColor:'#f7c450',fillOpacity:.14,interactive:false});circle.bindTooltip(town+' road-count area · '+radiusKm+' km',{permanent:true,direction:'center',className:'town-count-area-label'});circle.addTo(townCountAreaLayer);let back=document.getElementById('townCountMapReturn');if(!back){back=document.createElement('div');back.id='townCountMapReturn';back.className='town-count-map-return';mapCard.append(back)}back.innerHTML='<strong>Viewing road-count area</strong><button type="button">Back to progress</button>';back.querySelector('button').onclick=()=>{clearTownCountArea();activateRoadprintsScreen('progress')};map.invalidateSize(true);map.fitBounds(circle.getBounds(),{padding:[36,36],maxZoom:13})};setTimeout(draw,100)}
const activateRoadprintsScreenWithTownArea=activateRoadprintsScreen;activateRoadprintsScreen=function(screen){if(screen!=='map')clearTownCountArea();activateRoadprintsScreenWithTownArea(screen)}
