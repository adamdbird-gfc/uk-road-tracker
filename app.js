...
async function fetchTownInventory(town){const settlements=await loadSettlementInventoryIndex(),entry=settlements.get(normaliseSettlementName(town));if(!entry||entry.status!=='available')return null;const response=await fetch('settlement-inventories-v1/'+encodeURIComponent(entry.code)+'.json',{cache:'force-cache'});const inventory=await response.json().catch(()=>null);if(!response.ok||!Number.isFinite(Number(inventory?.count)))throw Error('Settlement inventory unavailable');return inventory}
...
function runSettlementChecks(){while(settlementCheckWorkers<2&&settlementCheckQueue.length){const road=settlementCheckQueue.shift();settlementCheckWorkers++;(async()=>{try{const names=new Set();for(const evidence of road.evidence||[]){const response=await fetch(API_BASE_URL+'/settlements-for-geometry',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({geometry:evidence.geometry})});if(!response.ok)throw Error('Boundary check unavailable');const data=await response.json();for(const settlement of data.settlements||[])if(settlement.name)names.add(settlement.name)}settlementCheckResults.set(road.id,{names:[...names]})}catch(_){settlementCheckResults.set(road.id,{error:true})}finally{settlementCheckPending.delete(road.id);settlementCheckWorkers--;renderRoadDiscovery();runSettlementChecks()}})()}}
function renderSettlementCheck(){const list=document.getElementById('roadDiscoveryList'),old=document.getElementById('settlementMatchingCheck');if(!list)return;old?.remove();const roads=[...roadDiscoveryLedger.values()].filter(road=>road.category==='Local roads');if(!roads.length)return;const panel=document.createElement('details'),summary=document.createElement('summary'),rows=document.createElement('ul');panel.id='settlementMatchingCheck';panel.className='road-discovery-group settlement-matching-check';summary.textContent='Settlement matching check (test) · '+roads.length;panel.append(summary,rows);for(const road of roads.sort((a,b)=>a.label.localeCompare(b.label,'en-GB'))){queueSettlementCheck(road);const result=settlementCheckResults.get(road.id),row=document.createElement('li'),label=document.createElement('strong'),state=document.createElement('span');label.textContent=road.label;const legacy=roadDiscoveryLocalityByBoundary(road);state.textContent=result?.error?'Check unavailable':!result?'Checking…':result.names.length?'ONS: '+result.names.join(', '):'ONS: no settlement';row.title='Legacy locality: '+legacy;row.append(label,state);rows.append(row)}list.before(panel)}
const renderRoadDiscoveryWithSettlementCheck=renderRoadDiscovery;
function renderBoundarySettlementLedger(){const list=document.getElementById('roadDiscoveryList'),roads=[...roadDiscoveryLedger.values()].filter(road=>road.category==='Local roads');if(!list||!roads.length||roads.some(road=>!settlementCheckResults.has(road.id)||settlementCheckResults.get(road.id).error))return;const legacy=[...list.querySelectorAll('.road-discovery-group')].find(group=>group.querySelector('summary')?.textContent.startsWith('Local roads'));if(!legacy)return;const towns=new Map();for(const road of roads)for(const town of settlementCheckResults.get(road.id).names)(towns.get(town)||towns.set(town,[]).get(town)).push(road);const group=document.createElement('details'),title=document.createElement('summary');group.className='road-discovery-group';title.textContent='Local roads · '+roads.length.toLocaleString();group.append(title);for(const town of [...towns.keys()].sort((a,b)=>a.localeCompare(b,'en-GB'))){const entries=towns.get(town),townDetails=document.createElement('details'),townTitle=document.createElement('summary'),rows=document.createElement('ul');townDetails.className='road-discovery-town';townTitle.dataset.town=town;townTitle.textContent=town+' · '+entries.length.toLocaleString();for(const road of entries.sort((a,b)=>a.label.localeCompare(b.label,'en-GB'))){const row=document.createElement('li'),label=document.createElement('strong'),state=document.createElement('span');label.textContent=road.label;state.textContent=road.driven&&road.onFoot?'Driven + on foot':road.driven?'Driven':'On foot';row.append(label,state);rows.append(row)}townDetails.append(townTitle,rows);group.append(townDetails);setTown(town,entries)}legacy.replaceWith(group)}
renderRoadDiscovery=function(){renderRoadDiscoveryWithSettlementCheck();renderSettlementCheck();renderBoundarySettlementLedger()};
