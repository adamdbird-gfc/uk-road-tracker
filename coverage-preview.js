...
(() => {
  ...
  let network,segments=[],grid=new Map(),savedDriving=[],onFoot=[],allOnFootActivities=[],map,control;
  ...
  async function readTimeline(){const file=input.files?.[0];if(!file)return;fileStatus.className='muted map-status';fileStatus.textContent=`Reading ${file.name}…`;
    try{allOnFootActivities=extractActivities(JSON.parse(await file.text()));onFoot=allOnFootActivities.filter(a=>a.points.some(inBounds)).slice(0,LIMIT);renderInventory(allOnFootActivities);card.classList.remove('hidden');matchButton.disabled=!onFoot.length;clearButton.disabled=!onFoot.length;
      fileStatus.textContent=onFoot.length?`${allOnFootActivities.length.toLocaleString()} usable walking/running activities inventoried; ${onFoot.length} in Gravesend are ready for the bounded test.`:`${allOnFootActivities.length.toLocaleString()} usable walking/running activities inventoried, but none enter the current Gravesend tile.`;render();
    }catch(error){fileStatus.className='error map-status';fileStatus.textContent=`Could not read this file: ${error.message||error}`;}}
  async function matchActivities(){const pending=onFoot.filter(a=>!a.geojson&&!a.error);if(!pending.length){fileStatus.className='muted map-status';fileStatus.textContent='All selected Gravesend activities have already been attempted. Clear the temporary result to run the sample again.';return;}matchButton.disabled=true;clearButton.disabled=true;let complete=0,succeeded=0,failed=0;
    try{for(const activity of pending){complete++;matchButton.textContent=`Matching ${complete} of ${pending.length}…`;fileStatus.className='muted map-status';fileStatus.textContent=`Matching Gravesend walking/running activity ${complete} of ${pending.length}. Keep this page open; each activity may take a few seconds.`;status.className='muted map-status';status.textContent=`Waiting for the pedestrian matcher (${complete}/${pending.length})…`;const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),60000);let response,data;try{response=await fetch(`${API_BASE_URL}/match-walking`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({points:activity.points}),signal:controller.signal});data=await response.json().catch(()=>({}));}finally{clearTimeout(timeout);}if(response.ok&&data.geojson?.features?.length){activity.geojson=data.geojson;succeeded++;}else{activity.error=data.detail||`HTTP ${response.status}`;failed++;}await new Promise(resolve=>setTimeout(resolve,1100));}render();fileStatus.className=failed?'muted map-status warn':'muted map-status ok';fileStatus.textContent=`Gravesend matching complete: ${succeeded} matched, ${failed} not matched. These results remain temporary.`;
    }catch(error){status.className='error map-status';status.textContent=`On-foot matching stopped: ${error.name==='AbortError'?'one activity took longer than 60 seconds':error.message||error}`;}finally{matchButton.disabled=false;clearButton.disabled=false;matchButton.textContent='Match Gravesend walking/running';}}
  function extractActivities(data){const semantic=Array.isArray(data?.semanticSegments)?data.semanticSegments:[],all=[];for(const segment of semantic)for(const item of Array.isArray(segment?.timelinePath)?segment.timelinePath:[]){const point=parsePoint(item?.point),time=Date.parse(item?.time||'');if(point&&Number.isFinite(time))all.push({...point,time});}all.sort((a,b)=>a.time-b.time);const result=[];for(const segment of semantic){const type=String(segment?.activity?.topCandidate?.type||'').toUpperCase();if(type!=='WALKING'&&type!=='RUNNING')continue;const start=Date.parse(segment.startTime||''),end=Date.parse(segment.endTime||'');if(!Number.isFinite(start)||!Number.isFinite(end))continue;const points=dedupe(all.filter(point=>point.time>=start&&point.time<=end).map(({lat,lng})=>({lat,lng})));if(points.length>1)result.push({points,start});}return result;}
  function renderInventory(activities){const grouped=new Map();for(const activity of activities){const centre=activity.points.reduce((sum,p)=>({lat:sum.lat+p.lat,lng:sum.lng+p.lng}),{lat:0,lng:0});centre.lat/=activity.points.length;centre.lng/=activity.points.length;const lat=Math.floor(centre.lat*10)/10,lng=Math.floor(centre.lng*10)/10,key=`${lat.toFixed(1)}°N, ${lng.toFixed(1)}°E`;if(!grouped.has(key))grouped.set(key,0);grouped.set(key,grouped.get(key)+1);}const areas=[...grouped.entries()].sort((a,b)=>b[1]-a[1]);$('coverageUsableActivities').textContent=activities.length.toLocaleString();$('coverageAreaCount').textContent=areas.length.toLocaleString();$('coverageGravesendActivities').textContent=activities.filter(a=>a.points.some(inBounds)).length.toLocaleString();const list=$('coverageAreaList');list.innerHTML='';for(const [name,count] of areas.slice(0,8)){const row=document.createElement('div');row.className='coverage-area-row';row.innerHTML=`<span>Approx. ${name}</span><strong>${count} activities</strong>`;list.append(row);}if(areas.length>8){const more=document.createElement('div');more.className='coverage-area-row';more.textContent=`${areas.length-8} further approximate areas`;list.append(more);}$('coverageInventory').classList.remove('hidden');}
